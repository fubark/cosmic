...
// Async curl should not use this handle since it will be from a worker thread.
var curl_h: Curl = undefined;

// Async curl.
var curl_mh: Curl = undefined;
...
pub fn getAsync(alloc: std.mem.Allocator, url: []const u8, timeout_secs: u65, keep_connection_open: bool) !Response {
    _ = timeout_secs;
    ...
    _ = curl_mh.setOption(curl.CURLOPT_SSL_VERIFYPEER, @intCast(c_long, 1));
    _ = curl_mh.setOption(curl.CURLOPT_SSL_VERIFYHOST, @intCast(c_long, 1));
    if (builtin.os.tag == .linux) {
        _ = curl_mh.setOption(curl.CURLOPT_CAINFO, "/etc/ssl/certs/ca-certificates.crt");
        _ = curl_mh.setOption(curl.CURLOPT_CAPATH, "/etc/ssl/certs");
    }

    _ = curl_mh.setOption(curl.CURLOPT_URL, c_url.ptr);
    _ = curl_mh.setOption(curl.CURLOPT_ACCEPT_ENCODING, "gzip, deflate, br");
    _ = curl_mh.setOption(curl.CURLOPT_WRITEFUNCTION, S.writeBody);
    _ = curl_mh.setOption(curl.CURLOPT_WRITEDATA, &buf);
    _ = curl_mh.setOption(curl.CURLOPT_HEADERFUNCTION, S.writeHeader);
    _ = curl_mh.setOption(curl.CURLOPT_HEADERDATA, &header_ctx);
    _ = curl_mh.setOption(curl.CURLOPT_TIMEOUT, timeout_secs);
    if (keep_connection_open) {
        _ = curl_mh.setOption(curl.CURLOPT_FORBID_REUSE, @intCast(c_long, 0));
    } else {
        _ = curl_mh.setOption(curl.CURLOPT_FORBID_REUSE, @intCast(c_long, 1));
    }
    
    _ = curl_mh.setOption(curl.CURLOPT_URL, c_url.ptr);

    _ = curl_m.addHandle(curl_mh);
    var running_handles: c_int = 1;
    while (running_handles > 0) {
        var res = curl_m.perform(&running_handles);
        if (res == curl.CURLE_OK and running_handles > 0) {
            res = curl_m.poll(null, 0, 1000, null);
        } 
        if (res != curl.CURLE_OK) {
            log.debug("failed, code {}", .{res});
            return error.RequestFailed;
        }
    }
    _ = curl_m.removeHandle(curl_mh);

    var http_code: u64 = 0;
    _ = curl_mh.getInfo(curl.CURLINFO_RESPONSE_CODE, &http_code);

    log.debug("async get {s}", .{buf.items});

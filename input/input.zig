...
pub const KeyboardEvent = struct {
    ...
    pub fn initWithMods(code: KeyCode, pressed: bool, mods: u8) @This() {
        return .{
            .code = code,
            .mods = mods,
            .pressed = pressed,
    ...
    pub fn init(code: KeyCode, pressed: bool, shift_pressed: bool, control_pressed: bool, alt_pressed: bool, meta_pressed: bool) Self {
        var mods: u8 = 0;
        if (shift_pressed) {
            mods |= ShiftMask;
        }
    ...
    pub fn getKeyChar(self: Self) u8 {
        // log.warn("FOO {} {} {}", .{@TypeOf(KeyCharMap), @enumToInt(self.code), self.mods & ShiftMask});
    ...
    pub fn isShiftPressed(self: *Self) bool {
        return self.mods & ShiftMask;
    }

    pub fn isControlPressed(self: *Self) bool {
        return self.mods & ControlMask;
    }

    pub fn isAltPressed(self: *Self) bool {
        return self.mods & AltMask;
    }

    pub fn isMetaPressed(self: *Self) bool {
        return self.mods & MetaMask;
    }
};

test "KeyboardEvent.getKeyChar" {
    const Shift = KeyboardEvent.ShiftMask;

    const S = struct {
        fn case(code: KeyCode, shift: bool, expected: u8) !void {
            const mods: u8 = if (shift) Shift else 0;
            try t.eq(KeyboardEvent.initWithMods(code, true, mods).getKeyChar(), expected);
        }
    };
    const case = S.case;

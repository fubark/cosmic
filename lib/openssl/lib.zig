...
pub fn createModule(b: *std.build.Builder) *std.build.Module {
    const mod = b.createModule(.{
        .source_file = .{ .path = thisDir() ++ "/openssl.zig" },
        .dependencies = &.{},
    });
    return mod;
    // step.addPackage(pkg);
...
/// in a new folder tree separated by target. The vendor directory should be simple and be one to one with the openssl repo.
pub fn createCryptoBuild(
    b: *std.build.Builder,
    ...
    // Don't include wincrypt.h from window.h which has conflicting declarations with crypto X509_EXTENSIONS.
    // Only when compiling to windows-gnu.
    if (target.getOsTag() == .windows and target.getAbi() == .gnu) {
        try c_flags.append("-DNOCRYPT=1");
    }
    ...
    for (c_files) |file| {
        const path = b.fmt("{s}/vendor/crypto/{s}", .{ thisDir(), file });
    ...
    for (digest_cfiles) |file| {
        const path = b.fmt("{s}/vendor/providers/{s}", .{ thisDir(), file });
        lib.addCSourceFile(path, c_flags.items);
    }
    ...
    // Point to either unix or windows config.
    // It's better to keep cflags small: https://github.com/ziglang/zig/issues/10693
    if (target.getOsTag() == .windows) {
        lib.addIncludePath(fromRoot(b, "windows/include"));
    } else {
        lib.addIncludePath(fromRoot(b, "unix/include"));
    }

    lib.addIncludePath(fromRoot(b, "vendor/providers/implementations/include"));
    lib.addIncludePath(fromRoot(b, "vendor/providers/common/include"));
    lib.addIncludePath(fromRoot(b, "vendor/include"));
    lib.addIncludePath(fromRoot(b, "vendor"));
...
pub fn buildAndLinkCrypto(step: *std.build.LibExeObjStep, opts: CryptoOptions) void {
    if (opts.lib_path) |path| {
        ...
        const lib = createCryptoBuild(step.builder, step.target, step.optimize) catch unreachable;
...
pub fn linkLibCrypto(step: *std.build.LibExeObjStep, lib: *std.build.LibExeObjStep) void {
    ...
    if (builtin.os.tag == .windows) {
        step.addAssemblyFile(thisDir() ++ "/../extras/prebuilt/win64/crypto.lib");
        return;
    }
    step.linkLibrary(lib);
}

pub fn linkLibCryptoPath(step: *std.build.LibExeObjStep, path: []const u8) void {
    linkLibCryptoDeps(step);
    step.addAssemblyFile(path);
}

fn linkLibCryptoDeps(step: *std.build.LibExeObjStep) void {
...
pub fn createSslBuild(
    ...
    // ssl doesn't seem to use opensslconf.h/configuration.h so some of this is duped.
    ...
    for (c_files) |file| {
        const path = b.fmt("{s}/vendor/ssl/{s}", .{ thisDir(), file });
        lib.addCSourceFile(path, c_flags.items);
    }
...
pub fn buildAndLinkSsl(step: *std.build.LibExeObjStep, opts: SslOptions) void {
    if (opts.lib_path) |path| {
        ...
        if (builtin.os.tag == .windows) {
            step.addAssemblyFile(thisDir() ++ "/../extras/prebuilt/win64/ssl.lib");
            return;
        }
        const lib = createSslBuild(step.builder, step.target, step.optimize) catch unreachable;
        linkLibSsl(step, lib);
    }
}
...
pub fn linkLibSslPath(step: *std.build.LibExeObjStep, path: []const u8) void {
    step.addAssemblyFile(path);
}

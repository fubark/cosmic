...
pub fn createCrypto(
    ...
    for (c_files) |file| {
        const path = b.fmt("{s}/vendor/crypto/{s}", .{ srcPath(), file });
    ...
    for (digest_cfiles) |file| {
        const path = b.fmt("{s}/vendor/providers/{s}", .{ srcPath(), file });
        lib.addCSourceFile(path, c_flags.items);
    }
...
pub fn buildAndLinkCrypto(step: *std.build.LibExeObjStep, opts: CryptoOptions) void {
    if (opts.lib_path) |path| {
        ...
        if (builtin.os.tag == .windows) {
            ...
            step.addAssemblyFile(srcPath() ++ "/../extras/prebuilt/win64/crypto.lib");
            return;
        }
...
pub fn createSsl(
    ...
    for (c_files) |file| {
        const path = b.fmt("{s}/vendor/ssl/{s}", .{ srcPath(), file });
        lib.addCSourceFile(path, c_flags.items);
    }
...
pub fn buildAndLinkSsl(step: *std.build.LibExeObjStep, opts: SslOptions) void {
    if (opts.lib_path) |path| {
        ...
        if (builtin.os.tag == .windows) {
            step.addAssemblyFile(srcPath() ++ "/../extras/prebuilt/win64/ssl.lib");
            return;
        }

function initGraphicsImports() {
    ...
    return {
        ...
        jsGlUniformMatrix3fv(location, transpose, value_ptr) {
            ...
            const wasm_view = new Uint8Array(wasm.exports.memory.buffer)
            ctx.uniformMatrix3fv(loc, transpose, new Float32Array(wasm_view.slice(value_ptr, value_ptr + 9*4).buffer))
        },
        ...
        jsGlUniform4fv(location, val_ptr) {
            const loc = uniform_locs.get(location)
            const wasm_view = new Uint8Array(wasm.exports.memory.buffer)
            ctx.uniform4fv(loc, new Float32Array(wasm_view.slice(val_ptr, val_ptr + 4*4).buffer))
        },
        jsGlUniform3fv(location, val_ptr) {
            const loc = uniform_locs.get(location)
            const wasm_view = new Uint8Array(wasm.exports.memory.buffer)
            ctx.uniform3fv(loc, new Float32Array(wasm_view.slice(val_ptr, val_ptr + 3*4).buffer))
        },
        jsGlUniform2fv(location, val_ptr) {
            const loc = uniform_locs.get(location)
            const wasm_view = new Uint8Array(wasm.exports.memory.buffer)
            ctx.uniform2fv(loc, new Float32Array(wasm_view.slice(val_ptr, val_ptr + 2*4).buffer))
        },
        jsGlUniform1fv(location, val_ptr) {
            const loc = uniform_locs.get(location)
            const wasm_view = new Uint8Array(wasm.exports.memory.buffer)
            ctx.uniform1fv(loc, new Float32Array(wasm_view.slice(val_ptr, val_ptr + 1*4).buffer))
        },
